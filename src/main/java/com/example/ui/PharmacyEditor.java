...
@SpringComponent
@UIScope
public class PharmacyEditor extends VerticalLayout {
    ...
    Binder<Pharmacy> binder = new Binder<>(Pharmacy.class);

    @Autowired
    public PharmacyEditor(PharmacyRepository repository) {
        ...
        delete.addClickListener(e -> repository.delete(pharmacy));
        cancel.addClickListener(e -> editCustomer(pharmacy));
        setVisible(false);
    }
    ...
    public final void editCustomer(Pharmacy c) {
        if (c == null) {
            setVisible(true);
            return;
        }
        ...
        if (persisted) {
            ...
            pharmacy = repository.findById(c.getId()).get();
        }
        else {
            pharmacy = c;
        }
